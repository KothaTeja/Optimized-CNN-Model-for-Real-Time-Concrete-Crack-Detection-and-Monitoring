...
def getContours(npImage, overlay_img, realHeight, realWidth, unit, confidence, angle_th=30):
    overlay_img = remove_blue_text(overlay_img)
    ...
    for c in cnts:
        ...
        if slope > 0:
            ...
            cv2.putText(overlay_img, "angle={:.1f}".format(angle), (max(top_p[0]-100, 0), top_p[1] + 15),
                        cv2.FONT_HERSHEY_DUPLEX, 0.45, (0, 0, 255), 1)
        else:
            angle = np.arccos((bot_p[0] - top_p[0])/D_ad) * 180 / math.pi
            cv2.putText(overlay_img, "angle={:.1f}".format(angle), (top_p[0], top_p[1] + 15),
                        cv2.FONT_HERSHEY_DUPLEX, 0.45, (0, 0, 255), 1)
        ...
        if angle < angle_th:
            cat += 'H'
            cv2.putText(overlay_img, "L={:.1f}".format(length) + unit, (int(tltrX), int(tltrY) + 100),
                        cv2.FONT_HERSHEY_DUPLEX, 0.45, (0, 0, 255), 1)
            cv2.putText(overlay_img, "W={:.1f}".format(width) + unit, (int(tltrX), int(tltrY) + 115),
                        cv2.FONT_HERSHEY_DUPLEX, 0.45, (0, 0, 255), 1)
        else:
            cat += 'V'
            cv2.putText(overlay_img, "L={:.1f}".format(length) + unit, (int(tltrX), int(tltrY)),
                        cv2.FONT_HERSHEY_DUPLEX, 0.45, (0, 0, 255), 1)
            cv2.putText(overlay_img, "W={:.1f}".format(width) + unit, (int(tltrX), int(tltrY) + 15),
                        cv2.FONT_HERSHEY_DUPLEX, 0.45, (0, 0, 255), 1)

        cat += 'L' if slope > 0 else 'R'

        cv2.putText(overlay_img, "Crack {:.2f}%".format(confidence.item()*100) + "   cat=" + cat,
                    (x1, max(0, y1 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (36, 255, 12), 1)
